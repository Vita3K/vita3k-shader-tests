...
void test_program(const SceGxmProgram *vert_program, float *input,
  ...
  memset(gxm_color_surfaces_addr[gxm_back_buffer_index], 0xDD, DISPLAY_WIDTH * DISPLAY_HEIGHT);
  ...
  err = sceGxmShaderPatcherCreateFragmentProgram(
      gxm_shader_patcher, fp_id, SCE_GXM_OUTPUT_REGISTER_FORMAT_FLOAT,
  ...
  float *pixels = (float *)gxm_color_surfaces_addr[gxm_back_buffer_index];
  float vo0 = pixels[0];
  float vo1 = pixels[DISPLAY_WIDTH - 1];
  float vo2 = pixels[DISPLAY_WIDTH * (DISPLAY_HEIGHT - 1)];
  float vo3 = pixels[DISPLAY_WIDTH * DISPLAY_HEIGHT - 1];
  output->vo0 = vo0;
  output->vo1 = vo1;
  output->vo2 = vo2;
  output->vo3 = vo3;
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < DISPLAY_BUFFER_COUNT; i++) {
    ...
    memset(gxm_color_surfaces_addr[i], 0xDD, DISPLAY_WIDTH * DISPLAY_HEIGHT);
    sceGxmColorSurfaceInit(
        &gxm_color_surfaces[i], SCE_GXM_COLOR_FORMAT_F32_R,
  ...
  printf("asdf\n");
    ...
    for (i = 0; i < 8; i++) {
      for (j = 0; j < 4; j++) {
        fprintf(res_fd, "sa%d: %x %f ", i*4+j, *(unsigned int *)&input[i*4 + j], input[i*4 + j]);
      }
    }
    fprintf(res_fd, "\n");
    fprintf(res_fd, "-------------------------------------------------------\n");
    for (j = 0; j < 4; j++) {
      ...
      for (i = 4; i < 4 * 9; i++) {
        fprintf(res_fd, "pa%d: %x %f ", i, *(unsigned int *)&input[4 * 9 * j + i], input[4 * 9 * j + i]);
      }
      fprintf(res_fd, "\n");
    }
    fprintf(res_fd, "-------------------------------------------------------\n");
    ...
    fprintf(res_fd, "-------------------------------------------------------\n");
    float res[4][3] = {{output.vo2, output1.vo2, output2.vo2},
    ...
    for (i = 0; i < 4; i++) {
      fprintf(res_fd, "vertex %d:\n", i);
      for (j = 0; j < 3; j++) {
        fprintf(res_fd, "o%d: %x %f ", 4 + 4 * j, *(unsigned int *)&res[i][j], res[i][j]);
      }
      fprintf(res_fd, "\n");
    }
